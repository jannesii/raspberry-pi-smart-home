...
class SensorsMixin:
    ...
    def migrate_sensors_to_pg(self, batch_size: int = 1000) -> dict[str, Any]:
        """
        Migrate sensor data from SQLite to PostgreSQL using bulk inserts.
        ...
        stats = {
            "esp32_temphum": {"migrated": 0, "errors": 0},
            "bmp_sensor_data": {"migrated": 0, "errors": 0},
        }

        logger.info("=" * 60)
        logger.info("Starting sensor data migration from SQLite to PostgreSQL")
        logger.info("Batch size: %d rows per transaction", batch_size)
        logger.info("=" * 60)

        # Migrate esp32_temphum in batches
        try:
            rows = self.db.fetchall(
                "SELECT id, location, timestamp, temperature, humidity, ac_on FROM esp32_temphum ORDER BY id"
            )
            total = len(rows)
            logger.info("📊 esp32_temphum: Found %d records to migrate", total)

            if total == 0:
                logger.info("✓ esp32_temphum: No records to migrate")
            else:
                ...
                for i, row in enumerate(rows, 1):
                    ...
                    if len(batch) >= batch_size or i == total:
                        try:
                            with sa_engine.begin() as conn:
                                conn.execute(insert(esp32_temphum), batch)
                            stats["esp32_temphum"]["migrated"] += len(batch)

                            # Calculate progress
                            elapsed = time.time() - start_time
                            progress_pct = (stats["esp32_temphum"]["migrated"] / total) * 100
                            rate = (
                                stats["esp32_temphum"]["migrated"] / elapsed if elapsed > 0 else 0
                            )
                            remaining = (
                                (total - stats["esp32_temphum"]["migrated"]) / rate
                                if rate > 0
                                else 0
                            )

                            logger.info(
                                "📈 esp32_temphum: %d/%d (%.1f%%) | %.0f rows/sec | ETA: %.0fs",
                                stats["esp32_temphum"]["migrated"],
                                total,
                                progress_pct,
                                rate,
                                remaining,
                            ...
                            logger.error("❌ Error migrating esp32_temphum batch: %s", e)
                            stats["esp32_temphum"]["errors"] += len(batch)
                            batch = []

                elapsed = time.time() - start_time
                logger.info(
                    "✓ esp32_temphum: Completed in %.1fs (%.0f rows/sec)",
                    elapsed,
                    total / elapsed if elapsed > 0 else 0,
                )

        except Exception as e:
            logger.exception("Error reading esp32_temphum from SQLite: %s", e)

        # Migrate bmp_sensor_data in batches
        try:
            rows = self.db.fetchall(
                "SELECT id, timestamp, temperature, pressure, altitude FROM bmp_sensor_data ORDER BY id"
            )
            total = len(rows)
            logger.info("📊 bmp_sensor_data: Found %d records to migrate", total)

            if total == 0:
                logger.info("✓ bmp_sensor_data: No records to migrate")
            else:
                start_time = time.time()
                batch = []
                for i, row in enumerate(rows, 1):
                    ...
                    if len(batch) >= batch_size or i == total:
                        try:
                            with sa_engine.begin() as conn:
                                conn.execute(insert(bmp_sensor_data), batch)
                            stats["bmp_sensor_data"]["migrated"] += len(batch)

                            # Calculate progress
                            elapsed = time.time() - start_time
                            progress_pct = (stats["bmp_sensor_data"]["migrated"] / total) * 100
                            rate = (
                                stats["bmp_sensor_data"]["migrated"] / elapsed if elapsed > 0 else 0
                            )
                            remaining = (
                                (total - stats["bmp_sensor_data"]["migrated"]) / rate
                                if rate > 0
                                else 0
                            )

                            logger.info(
                                "📈 bmp_sensor_data: %d/%d (%.1f%%) | %.0f rows/sec | ETA: %.0fs",
                                stats["bmp_sensor_data"]["migrated"],
                                total,
                                progress_pct,
                                rate,
                                remaining,
                            ...
                            logger.error("❌ Error migrating bmp_sensor_data batch: %s", e)
                            stats["bmp_sensor_data"]["errors"] += len(batch)
                            batch = []

                elapsed = time.time() - start_time
                logger.info(
                    "✓ bmp_sensor_data: Completed in %.1fs (%.0f rows/sec)",
                    elapsed,
                    total / elapsed if elapsed > 0 else 0,
                )

        except Exception as e:
            logger.exception("Error reading bmp_sensor_data from SQLite: %s", e)

        logger.info("=" * 60)
        logger.info("✓ Sensor migration complete!")
        logger.info(
            "  esp32_temphum: %d migrated, %d errors",
            stats["esp32_temphum"]["migrated"],
            stats["esp32_temphum"]["errors"],
        )
        logger.info(
            "  bmp_sensor_data: %d migrated, %d errors",
            stats["bmp_sensor_data"]["migrated"],
            stats["bmp_sensor_data"]["errors"],
        )
        logger.info("=" * 60)
        return stats
