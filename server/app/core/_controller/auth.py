...
class AuthMixin:
    ...
    def migrate_auth_to_pg(self, batch_size: int = 1000) -> dict[str, Any]:
        """
        Migrate auth data (users and API keys) from SQLite to PostgreSQL using bulk inserts.
        ...
        stats = {
            ...
            "api_keys": {"migrated": 0, "errors": 0},
        }

        logger.info("=" * 60)
        logger.info("Starting auth data migration from SQLite to PostgreSQL")
        ...
        try:
            rows = self.db.fetchall(
                "SELECT id, username, password_hash, is_admin, is_root_admin, is_temporary, expires_at FROM users ORDER BY id"
            )
            total = len(rows)
            logger.info("📊 users: Found %d records to migrate", total)

            if total == 0:
                logger.info("✓ users: No records to migrate")
            else:
                ...
                for i, row in enumerate(rows, 1):
                    ...
                    if len(batch) >= batch_size or i == total:
                        try:
                            ...
                            stats["users"]["migrated"] += len(batch)

                            elapsed = time.time() - start_time
                            progress_pct = (stats["users"]["migrated"] / total) * 100
                            rate = stats["users"]["migrated"] / elapsed if elapsed > 0 else 0
                            remaining = (
                                (total - stats["users"]["migrated"]) / rate if rate > 0 else 0
                            )

                            logger.info(
                                "📈 users: %d/%d (%.1f%%) | %.0f rows/sec | ETA: %.0fs",
                            ...
                            logger.error("❌ Error migrating users batch: %s", e)
                            stats["users"]["errors"] += len(batch)
                            batch = []

                elapsed = time.time() - start_time
                logger.info(
                    "✓ users: Completed in %.1fs (%.0f rows/sec)",
                    elapsed,
                    total / elapsed if elapsed > 0 else 0,
                )

        except Exception as e:
            logger.exception("Error reading users from SQLite: %s", e)

        # Migrate api_keys
        try:
            rows = self.db.fetchall(
                "SELECT id, key_id, name, secret_hash, created_at, created_by, revoked, last_used_at FROM api_keys ORDER BY id"
            )
            total = len(rows)
            logger.info("📊 api_keys: Found %d records to migrate", total)

            if total == 0:
                logger.info("✓ api_keys: No records to migrate")
            else:
                start_time = time.time()
                batch = []
                for i, row in enumerate(rows, 1):
                    ...
                    if len(batch) >= batch_size or i == total:
                        try:
                            with sa_engine.begin() as conn:
                                conn.execute(insert(api_keys), batch)
                            stats["api_keys"]["migrated"] += len(batch)

                            elapsed = time.time() - start_time
                            progress_pct = (stats["api_keys"]["migrated"] / total) * 100
                            rate = stats["api_keys"]["migrated"] / elapsed if elapsed > 0 else 0
                            remaining = (
                                (total - stats["api_keys"]["migrated"]) / rate if rate > 0 else 0
                            )

                            logger.info(
                                "📈 api_keys: %d/%d (%.1f%%) | %.0f rows/sec | ETA: %.0fs",
                                stats["api_keys"]["migrated"],
                                total,
                                progress_pct,
                                rate,
                                remaining,
                            ...
                            logger.error("❌ Error migrating api_keys batch: %s", e)
                            stats["api_keys"]["errors"] += len(batch)
                            batch = []

                elapsed = time.time() - start_time
                logger.info(
                    "✓ api_keys: Completed in %.1fs (%.0f rows/sec)",
                    elapsed,
                    total / elapsed if elapsed > 0 else 0,
                )

        except Exception as e:
            logger.exception("Error reading api_keys from SQLite: %s", e)

        logger.info("=" * 60)
        logger.info("✓ Auth migration complete!")
        logger.info(
            "  users: %d migrated, %d errors", stats["users"]["migrated"], stats["users"]["errors"]
        )
        logger.info(
            "  api_keys: %d migrated, %d errors",
            stats["api_keys"]["migrated"],
            stats["api_keys"]["errors"],
        )
        logger.info("=" * 60)
        return stats
