...
if TYPE_CHECKING:
    ...
    from .car_heater.status import FallbackStatus
...
def parse_timestamp(raw_ts: str) -> datetime:
    ...
    dt_utc = datetime.strptime(raw_ts, "%Y-%m-%d %H:%M:%S").replace(tzinfo=datetime.UTC)
...
def run_keep_at_temp_tick(car: CarHeaterStatus) -> None:
    ...
    logger.debug("run_keep_at_temp_tick called with car: %s", car)
    try:
        svc: KeepAtTempService | None = getattr(current_app, "keep_at_temp_service", None)
        logger.debug("run_keep_at_temp_tick resolved service: %s", svc)
        if svc:
...
def handle_status_update_request(
    ...
    logger.debug(
        "Parsed status update timestamp=%s shelly_connected=%s ambient_temp=%s",
        timestamp,
        shelly_connected,
        ambient_temp,
    )
            ...
            if power_w < _ALERT_MIN_POWER_W:
                ...
                if low_power_s >= _ALERT_NO_POWER_MINUTES * 60:
                    record_alert(
                        ...
                        message=(f"power_w={power_w:.1f} " f"duration_min={low_power_s/60:.1f}"),
                    )
            else:
        ...
        if last_ts is not None:
            ...
            if missing_s >= _ALERT_SHELLY_MISSING_MINUTES * 60:
                ...
                record_alert(
                    ...
                    message=(f"missing_min={missing_s/60:.1f} " f"last_heater_on={last_heater}"),
                )
