...
def handle_log_stream(handler: SocketEventHandler, data: dict[str, Any]) -> None:
    ...
    if not getattr(current_user, "is_admin", False):
        handler.socketio.emit("error", {"message": "Admin required for log stream"})
        return
    ...
    try:
        if action == "subscribe":
            ...
            logger.info("Log stream subscriber added: %s (lines=%d)", sid, lines)
            ...
            handler.socketio.emit("error", {"message": f"Invalid log_stream action: {action}"})
            ...
            for line in iter(handler._log_stream_process.stdout.readline, ""):
                ...
                if line:
                    ...
                    for sub_sid in list(handler._log_stream_sids):
                        try:
                            handler.socketio.emit("log_line", {"line": line}, to=sub_sid)
                        except Exception:
...
def cleanup_log_stream(handler: SocketEventHandler) -> None:
    ...
    logger.debug("cleanup_log_stream called")
    if handler._log_stream_process is not None:
        try:
