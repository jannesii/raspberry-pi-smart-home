...
class SessionManager:
    ...
    def start_session(
        ...
        *,
        ...
        logger.info(
            ...
            mode, outside_temp_c, cabin_temp_c,
        )
    ...
    def append_sample(
        ...
        *,
        ...
        logger.debug(
            ...
            ts.isoformat(), cabin_temp_c, heater_on, power_w, outside_temp_c,
    ...
    def finalize_session(
        ...
        *,
        ...
        informative, info_details = self._is_informative_session(
            samples=samples, started=started, ended=end_ts,
        )
        if not informative:
        ...
        fit = self._physics.fit_params(
            samples, active_before_k, active_before_eta) if informative else None
        ...
        if fit is not None and accepted:
            new_k, new_eta, alpha = self._compute_weighted_update(
                old_k=active_before_k, old_eta=active_before_eta, fit=fit, quality_score=quality,
            )
            active_after = {"k_loss_W_per_K": new_k,
                            "eta": new_eta, "alpha": alpha}
            promoted = True
            ...
            self._update_bucket_params(
                fit=fit, outside_mean=outside_mean, wind_mean=wind_mean,
                updated_at=end_ts, is_test=is_test,
            )
        ...
        if fit is not None and accepted:
            logger.info(
                ...
                quality, fit.rmse_C, fit.k_loss_W_per_K, fit.eta,
            )
        else:
            logger.info(
                "kfactor: session rejected (quality=%.2f duration=%ss deltaT=%.2f reason=%s)",
                quality, duration_s, delta_t, reason,
            )
        ...
        if is_test:
            self._append_test_output({
                ...
                "active_params_before": {"k_loss_W_per_K": active_before_k, "eta": active_before_eta},
    ...
    def _quality_score(
        ...
        *,
        ...
        disturbance = any(k.startswith("non_monotonic")
                          or k.startswith("spike") for k in flags.keys())
    ...
    def _update_bucket_params(
        ...
        *,
        ...
        from app.core import CarHeaterKFactorBucketParams
    ...
    def _update_active_params(
        ...
        *,
        ...
        from app.services.alert_webhook import record_alert
        from app.core import CarHeaterKFactorActiveParams
        ...
        new_k, new_eta, _alpha = self._compute_weighted_update(
            old_k=old_k, old_eta=old_eta, fit=fit, quality_score=quality_score,
        )
